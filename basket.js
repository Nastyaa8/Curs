...
var cartModule = (function () { // пространство имён
    ...
    var cart = {
            ...
            if(Object.keys(data).length !== 0){
                ...
                for(let items in data){
                    ...
                    item += '<tr class="quantity" data-id="' + data[items][0] + '">'
                         ...
                         + '<td class="titleProduct" style="color: #fff; font-size: 16px;">' + data[items][1] + '</td>'
                         + '<td class="priceProduct">' + data[items][2] + '</td>'
                         + '<td><button class="decrement btn btn-primary-left">-</button> <button class="count btn btn-default">' + data[items][3] + '</button> <button class="increment btn btn-primary-right">+</button></td>'
                         + '<td class="delete"><button class="btn btn-default-basket">Удалить</button></td>'
                         + '</tr>';
                }
                ...
                cart.getContent().innerHTML = ''
                    ...
                    + item
                    +'<tr><td></td><td class="total" style="font-size: 18px;" >ИТОГО:</td><td class="total" style="font-size: 18px;" >' + price + '</td><td class="total" style="font-size: 18px;" >' + count + '</td><td><button id="clear_cart" class="btn btn-default-basket" style="font-size: 18px;" >Очистить корзину</button></td></tr>'
                    + '</table>';
        ...
        quantity : function () { // Увеличение, уменьшение количества товара
            let item = {
                ...
                title    : this.parentNode.parentNode.querySelector('.titleProduct').innerHTML,
                price    : this.parentNode.parentNode.querySelector('.priceProduct'),
        ...
        checkout : function () {
            ...
            this.getCheckout().innerHTML = 'Смотреть корзину <sup>' + parseInt(localStorage.getItem('RudraJS-Cart::count')) + '</sup>';
        },
